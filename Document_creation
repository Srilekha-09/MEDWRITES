...
def create(name, age, date, tablet):
    ...
    run = para.add_run(
        " Dr.Aman M.D.(Dermatology) \t \t Dr.Swathy M.S.(Ortho) \t Dr.Shreevarsha M.S.(Ophthalmology) \n No.20,Blossom Avenue,Madura \n Ph:0452-2588 1522 \n")
    # Set page attributes
    font = run.font
    ...
    if (len(tablet) == 0):
        ...
        table = document.add_table(len(tablet), 10)

        heading_cells = table.rows[0].cells
        heading_cells[0].text = 'Name'
        heading_cells[1].text = 'Medication name with RxNORM'
        heading_cells[2].text = 'Dosage and Strength'
        heading_cells[3].text = 'Route'
        heading_cells[3].text = 'Duration and Frequency'
...
if __name__ == "__main__":
    create("new_test", "20", "12-11-2022", [["tab1", "10", "3", "After", "1-0-1"]], [])
