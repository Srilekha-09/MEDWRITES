...
def verify(name=None):
	...
	if name:
		p = "F:/Mini Projects srilekha/voice prescription/{}.docx".format(name)
	# Manual verification of existing files
	else:
		filename = filedialog.askopenfilename(initialdir="F:\\Mini Projects srilekha\\voice prescription", title="Select file", filetypes=(("document files", "*.docx"), ("all files", "*.*")))
		p = filename
...
def doc():
	...
	filename = filedialog.askopenfilename(initialdir="F:\\Mini Projects srilekha\\voice prescription", title="Select file", filetypes=(("wav files", "*.wav"),("all files", "*.*")))
